...
def sources2D_nwb():
    ...
    def __init__(self):
        ...
        # get the sources2D handle path
        root = rf.dropbox_root(dropbox_folder='timspellman')
        sources2d_path = os.path.join(root,r"C:\Users\johnj\SpellmanLab Dropbox\timspellman\MATLAB\SpellmanLab_SharedScripts\CNMF_E\ca_source_extraction")
        pass
...
class behavior_nwb():
    ...
    def readwrite(self, existing_nwb_path: str):
        ...
        with NWBHDF5IO(existing_nwb_path, "r+") as io:
            ...
            congruent = (self.behdict['trialLRs'][0] == self.behdict['irrelLRs'][0]).astype(int)
            ol_stim = ( ((self.behdict['setIDs'][0]==0) & (self.behdict['trialLRs'][0]==1)) | ((self.behdict['setIDs'][0]==1) & (self.behdict['irrelLRs'][0]==1)) ).astype(int)
            wh_stim = ( ((self.behdict['setIDs'][0]==1) & (self.behdict['trialLRs'][0]==1)) | ((self.behdict['setIDs'][0]==0) & (self.behdict['irrelLRs'][0]==1)) ).astype(int)
            ...
            nwbfile.add_trial_column(name="congruentTrial",        description="Boolean: (1 == relevant and irrelevant stimuli are on the same direction [LR], 0 == relevant and irrelevant stimuli are on opposite directions)")
            nwbfile.add_trial_column(name="olfactoryStimLR",       description="Boolean: (1 == olfactory stimulus present on the right, 0 = olfactory stimulus on the left)")
            nwbfile.add_trial_column(name="whiskerStimLR",         description="Boolean: (1 == whisker stimulus present on the right, 0 = whisker stimulus on the left)")
...
class unwrap():
    ...
    def spellmanBeh(self, nwbpath: str):
        ...
        # lick times left and right
        behdict['lickTimesR']   = [np.array(nwbfile.intervals['lickR_times'].to_dataframe())[:,0]]
        behdict['lickTimesL']   = [np.array(nwbfile.intervals['lickL_times'].to_dataframe())[:,0]]

        # recreated from lick times Left and Right
        behdict['lickTimesAll'] = np.sort(np.concatenate((behdict['lickTimesL']+behdict['lickTimesR']),axis=0))

        # index of start and stop times per trial
        behdict['trialStartTimes'] = [np.array(trial_data.trialStartIdx)]   
        behdict['trialEndTimes']   = [np.array(trial_data.trialEndIdx)]   

        # index of stimulus onset and offset times per trial
        behdict['stimOnTimes']  = [np.array(trial_data.stimulusOnsetIdx)]
        behdict['stimOffTimes'] = [np.array(trial_data.stimulusOffsetIdx)]

        # index of reward times - going to add something here
        behdict['rewardTimesIdxTrials'] = [np.array(trial_data.trialRewardTimesIdx)]
        behdict['rewardTimes'] = [behdict['rewardTimesIdxTrials'][0][~np.isnan(behdict['rewardTimesIdxTrials'][0])].astype(int)]

        # frameTimes
        behdict['frameTimes'] = [nwbfile.acquisition['frameTimes'].data[:]]

        # get count data
        behdict['lickNumL']       = [np.array(trial_data.LickCountsL)]
        behdict['lickNumR']       = [np.array(trial_data.LickCountsR)]
        behdict['irrelLRs']       = [np.array(trial_data.irrelevantLR)]
        behdict['trialLRs']       = [np.array(trial_data.relevantLR)]
        behdict['setIDs']         = [np.array(trial_data.setShiftingID)]
        behdict['trialCorrect']   = [np.array(trial_data.choiceOutcome)]
        behdict['congruentTrial'] = [np.array(trial_data.congruentTrial)]
