...
Edit:
    10/21/2024 - John added io for C and S variables from denoising and cleaning raw traces using caiman techniques
"""
import os, time
import numpy as np
import scipy.io
from scipy.interpolate import interp1d # JS added on 10/21/2024 due to missing function
...
def load_files(name):
    ...
    if stat is not None:
        ...
        try:
            ...
            try: 
                Ccell = np.load(basename + "/C.npy")
                Scell = np.load(basename + "/S.npy")
                print("Loaded C and S")
            except:
                Ccell = np.array([])
                Scell = np.array([])
                print("No detected C and S variables...")
    ...
    if goodfolder:
        # JS
        return stat, ops, Fcell, Fneu, Spks, iscell, probcell, redcell, probredcell, hasred, Ccell, Scell
    else:
...
def load_to_GUI(parent, basename, procs):
    # JS
    stat, ops, Fcell, Fneu, Spks, iscell, probcell, redcell, probredcell, hasred, Ccell, Scell = procs
...
def save_merge(parent):
    ...
    np.save(os.path.join(parent.basename, "S.npy"), parent.S) # JS
    np.save(os.path.join(parent.basename, "C.npy"), parent.C) # JS    
