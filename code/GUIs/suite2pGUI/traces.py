...
def plot_trace(parent):
    ...
    if len(parent.imerge) == 1:
        n = parent.imerge[0]
        f = zscore(parent.Fcell[n, :])
        fneu = zscore(parent.Fneu[n, :])
        sp = zscore(parent.Spks[n, :])
        # JS
        S = zscore(parent.S[n, :])
        C = zscore(parent.C[n, :])
        if np.ptp(fneu) == 0:
        ...
        # JS
        if parent.COn:
            parent.p3.plot(parent.trange, C, pen="y")
        if parent.SOn:
            parent.p3.plot(parent.trange, S, pen="b")
        parent.fmin = fmin
        parent.fmax = fmax
        ax.setTicks(None)
    else:
...
def make_buttons(parent, b0):
    ...
    # JS
    parent.comboBox.addItem("C (denoised fluorescence)")
    parent.comboBox.addItem("S (deconvolved spikes on C)")
    ...
    # JS
    # denoised fluorescence CHECKBOX
    parent.l0.setVerticalSpacing(4)
    parent.checkBoxC = QCheckBox("Denoised Fluorescence (C)")
    parent.checkBoxC.setStyleSheet("color: yellow;")
    parent.checkBoxC.toggled.connect(lambda: c_on(parent))
    parent.COn = True
    parent.checkBoxC.toggle()
    parent.l0.addWidget(parent.checkBoxC, b0, 9, 1, 2)

    # denoised and deconvolved fluorescence CHECKBOX
    parent.l0.setVerticalSpacing(4)
    parent.checkBoxS = QCheckBox("Denoised and deconvolved fluorescence (S)")
    parent.checkBoxS.setStyleSheet("color: blue;")
    parent.checkBoxS.toggled.connect(lambda: s_on(parent))
    parent.SOn = False
    parent.checkBoxS.toggle()
    parent.l0.addWidget(parent.checkBoxS, b0, 11, 1, 2)
...
def make_buttons(parent, b0):
    ...
    parent.comboBox.addItem("deconvolved")
    # JS
    parent.comboBox.addItem("C (denoised flourescence)")
    parent.comboBox.addItem("S (deconvolved spks on C)")
    parent.activityMode = 3
    parent.comboBox.setCurrentIndex(parent.activityMode)
    parent.comboBox.currentIndexChanged.connect(parent.mode_change)
    ...
    # denoised flourescence CHECKBOX
    parent.l0.setVerticalSpacing(4)
    parent.checkBoxt = QCheckBox("Denoised Flourescence (C)")
    parent.checkBoxt.setStyleSheet("color: yellow;")
    parent.checkBoxt.toggled.connect(lambda: c_on(parent))
    parent.tracesOn = True
    parent.checkBoxt.toggle()
    parent.l0.addWidget(parent.checkBoxt, b0, 9, 1, 2)

    # denoised flourescence CHECKBOX
    parent.l0.setVerticalSpacing(4)
    parent.checkBoxt = QCheckBox("Denoised and deconvolved flourescence (S)")
    parent.checkBoxt.setStyleSheet("color: blue;")
    parent.checkBoxt.toggled.connect(lambda: s_on(parent))
    parent.tracesOn = True
    parent.checkBoxt.toggle()
    parent.l0.addWidget(parent.checkBoxt, b0, 11, 1, 2)    
    return b0
'''
