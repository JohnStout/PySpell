...
self.build_classifier(preset_features=False, grid_search=False)
#self.save_model(filepath=os.path.join(rootfun.dropbox_root(),'OtherData','ClassifierBuildSuite2p','cellClassifier_grid_weightedSkew.pkl'))
#self.classify(session_path=r"C:\Users\johnj\SpellmanLab Dropbox\OtherData\John\EXPERIMENTS\LAYER6\Subjects\Imaging\L608_F_LeftPFC_L6Chr_PFCgcamp6f_L6PAN\SEDS_day2_FOV1_LBC0_optoRec_img\suite2p\plane0")
...
class cellClassifier():
    ...
    def build_classifier(self,
        ...
        • per‐session scaling + new skew/comp_SNR flags
        """
        import numpy as np
        from imblearn.pipeline    import Pipeline as ImbPipeline
        from imblearn.over_sampling import SMOTE
        ...
        # 0b) Add skew‐flags (binary features)
        skew_thresholds = [0.8, 0.9, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0]
        for t in skew_thresholds:
            df_clean[f"skewF_lt_{t}"] = (df_clean["skewF"] < t).astype(int)

        p80, p85, p90, p95, p99 = np.percentile(df_clean["comp_SNR"], [80, 85, 90, 95, 99])
        for q, thresh in zip([80, 85, 90, 95, 99], [p80, p85, p90, p95, p99]):
            # e.g. compSNR_perc_90 == 1 if comp_SNR is BELOW the 90th‑pct threshold
            df_clean[f"compSNR_perc_{q}"] = (df_clean["comp_SNR"] < thresh).astype(int)

        # 1) Tag & split off features/labels
        groups = df_clean["mouseName"]
        X      = df_clean.drop(columns=["iscell","mouseName"])
        y      = df_clean["iscell"]

        # 2) Feature subset logic
        if preset_features:
            flist = feature_list or ['comp_SNR','skewF','fitness','corr',
                                    'compact','npix_norm','npix']
            X = X[flist]
        elif feature_list:
            X = X[feature_list]
        ...
        # 3) Split into train/test, with optional per‐session scaling
        if per_session_scaling:
            gss = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=0)
            tr_idx, te_idx = next(gss.split(X, y, groups))
            X_train, X_test = X.iloc[tr_idx], X.iloc[te_idx]
            y_train, y_test = y.iloc[tr_idx], y.iloc[te_idx]
            grp_tr, grp_te = groups.iloc[tr_idx], groups.iloc[te_idx]

            # per‐session scalers
            self.session_scalers = {}
            Xtr_s = pd.DataFrame(index=X_train.index, columns=X_train.columns)
            for sess in grp_tr.unique():
                m = grp_tr==sess
                sc = StandardScaler().fit(X_train.loc[m])
                Xtr_s.loc[m] = sc.transform(X_train.loc[m])
                self.session_scalers[sess] = sc

            # global fallback
            self.global_scaler = StandardScaler().fit(X_train)
            Xte_s = pd.DataFrame(index=X_test.index, columns=X_test.columns)
            for sess in grp_te.unique():
                m = grp_te==sess
                sc = self.session_scalers.get(sess, self.global_scaler)
                Xte_s.loc[m] = sc.transform(X_test.loc[m])

            X_train, X_test = Xtr_s, Xte_s
        ...
        # 3b) Add comp_SNR percentile flags (train only → no leakage)
        p90, p95, p99 = np.percentile(X_train["comp_SNR"], [90,95,99])
        for p,val in zip([90,95,99],[p90,p95,p99]):
            X_train[f"comp_SNR_gt_{p}"] = (X_train["comp_SNR"] > val).astype(int)
            X_test [f"comp_SNR_gt_{p}"] = (X_test ["comp_SNR"] > val).astype(int)

        # update selected_features to include them
        self.selected_features = list(X_train.columns)

        # 4) Optional RFE
        if auto_feature_select and not preset_features:
            print("Running RFE to select top features…")
            svc_tmp = SVC(kernel='linear', class_weight='balanced')
            best_score, best_support = -1, None
            for k in range(1, X_train.shape[1]+1):
                rfe = RFE(svc_tmp, n_features_to_select=k)
                score = cross_val_score(rfe, X_train, y_train, cv=5).mean()
                if score > best_score:
                    best_score   = score
                    best_support = rfe.fit(X_train, y_train).support_
        ...
        if grid_search:
            ...
            param_grid = [
                ...
                # … add your other estimator grids similarly …
            ]
            gs = GridSearchCV(pipeline, param_grid, cv=5, scoring='accuracy',
                            n_jobs=-1, verbose=1)
            gs.fit(X_train, y_train)
            print("Best pipeline:", gs.best_params_)
        ...
        # 6) Manual fallback
            ...
            print("Manual: scale → SMOTE → PCA → SVC")
            scaler = StandardScaler().fit(X_train)
            Xt, Xv = scaler.transform(X_train), scaler.transform(X_test)
            sm     = SMOTE(random_state=0)
            Xt, yt = sm.fit_resample(Xt, y_train)
            pca    = PCA(n_components=min(Xt.shape[1]-1,5))
            Xt, Xv = pca.fit_transform(Xt), pca.transform(Xv)
...
class SkewFilter(BaseEstimator, TransformerMixin):
    def __init__(self, threshold=0.8, col_raw="skF", col_dn="skFc"):
    ...
    def transform(self, X):
        mask = (X[self.col_raw]  >= self.threshold) & \
            (X[self.col_dn]   >= self.threshold)
        return X.loc[mask]
