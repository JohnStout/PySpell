    ...
    def convert(self, method: str = 'suite2p', split_file = True):
        ...
        if '4D' in method:
            ...
            for zi in range(z):
                ...
                plane_array = self.vector_list[zi::z]

                for timei in time_range[::chunker]:
                    im[zi,timei:timei+chunker,:,:] = plane_array[timei:timei+chunker] 
                    im.flush()
                ...
                # TEST
                #import matplotlib.pyplot as plt
                #plane_array = self.vector_list[0::z]
                #plane_array1 = self.vector_list[1::z]
                #plane_array2 = self.vector_list[2::z]
                #idx=-1 #CHANGE ME
                #fig,ax=plt.subplots(nrows=1,ncols=z)
                #ax[0].imshow(plane_array[idx])
                #ax[1].imshow(plane_array1[idx])
                #ax[2].imshow(plane_array2[idx])
            ...
            for framesi in range(len(chunk_loop)):
                temp = self.vector_list[chunk_loop[framesi]:chunk_loop[framesi]+(chunker*z)] # get temporary data
                planes = []
                for zi in range(z):
                    planes.append(temp[zi::z]) # automatically establish the sampling over planes and data to extract every plane as separate variables                   
                # keep track of true samples
                true_samples = len(planes[0])
                # chunky write :) - no need to worry about the remainder bc slicing takes care of it!
                im[time_loop[framesi]:time_loop[framesi]+true_samples,:,:] = np.max(np.array(planes),axis=0)
                im.flush()
                del im; im=tf.memmap(self.fname)
