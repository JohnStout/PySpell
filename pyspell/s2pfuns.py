...
class cellClassifier():
    ...
    def build_classifier(self,
                        ...
                        per_session_scaling:    bool = True,
                        drop_nan:               bool = True,
                        skew_classifier:        bool = False,
                        skew_edge_low:          float = 0.8,
                        skew_edge_high:         float = 1.2   ## ← NEW
                        ):
        """
        ...
        A general classifier is built and run on training data using a ton of parameters.
        A secondary classifier is built, run, and tested after zooming in on edge-cases for skew to improve classification
        """
        from imblearn.pipeline    import Pipeline as ImbPipeline
        from imblearn.over_sampling import SMOTE
        from sklearn.preprocessing import StandardScaler
        ...
        self.skew_classifier = skew_classifier
        self.skew_edge_low   = skew_edge_low      # same defaults as your build method
        self.skew_edge_high  = skew_edge_high

        # 0) clean up and tag each row with its session
        df_clean, idx_rem = self.cleanup_classifier_data(self.df_train, drop_nan = drop_nan)
        ...
        # 1) feature‐subset logic (unchanged)
        if preset_features:
            flist = feature_list or ['comp_SNR','skewF','fitness','corr','compact','npix_norm','npix','pow_signal_noise','asymmetry','pnr']
            X = X[flist]
        elif feature_list:
            X = X[feature_list]
        self.selected_features = list(X.columns)
        ...
        # Build edge‐case classifier on edge‐skewed cells
        # use .iloc on the original df_clean's 'skewF' to match train_idx positions
        skew_train = df_clean['skewF'].iloc[train_idx]
        mask_edge  = (skew_train > skew_edge_low) & (skew_train < skew_edge_high)
        if mask_edge.sum() > 20 & skew_classifier==True:
            X_edge = X_train.loc[mask_edge]
            y_edge = y_train.loc[mask_edge]

            if grid_search:

                # --- 1) build your “edge‐specialist” pipeline skeleton ---
                edge_base = ImbPipeline([
                    ('scaler', StandardScaler()),
                    ('smote',   SMOTE(random_state=0)),  # will be toggled in the grid
                    ('pca',     PCA()),                  # likewise
                    ('clf',     SVC(class_weight='balanced', probability=True))
                ])

                # --- 2) param grid uses the string 'passthrough' to disable steps ---
                param_grid_edge = {
                    'smote':             [SMOTE(random_state=0),
                                        BorderlineSMOTE(random_state=0),
                                        'passthrough'],    # ← skip SMOTE
                    'pca':               [PCA(n_components=min(X_edge.shape[1]-1,5)),
                                        'passthrough'],    # ← skip PCA
                    'clf__kernel':       ['linear','rbf'],
                    'clf__C':            [0.1, 1, 10],
                    'clf__gamma':        ['scale','auto'],
                }

                # --- 3) small inner CV, optimizing recall for the “True” cells in the edge band ---
                inner_cv = StratifiedKFold(n_splits=3, shuffle=True, random_state=0)
                gs_edge = GridSearchCV(
                    edge_base,
                    param_grid_edge,
                    cv=inner_cv,
                    scoring='recall',    # maximize recall on True cells in the skew‐edge
                    n_jobs=-1,
                    verbose=1
                )
                gs_edge.fit(X_edge, y_edge)
                self.edge_pipeline = gs_edge.best_estimator_

                # --- 4) finally evaluate on your held‐out edge‐band test samples ---
                skew_test      = df_clean['skewF'].iloc[test_idx]
                mask_edge_test = (skew_test > skew_edge_low) & (skew_test < skew_edge_high)
                if mask_edge_test.any():
                    Xe_test = X_test.loc[mask_edge_test]
                    ye_test = y_test.loc[mask_edge_test]
                    ye_pred = self.edge_pipeline.predict(Xe_test)
                    print("Edge‐case specialist recall:", recall_score(ye_test, ye_pred))
                    print(classification_report(ye_test, ye_pred))

            else:
                sc2      = StandardScaler().fit(X_edge)
                Xe       = sc2.transform(X_edge)
                sm2      = SMOTE(random_state=0)
                Xe2, ye2 = sm2.fit_resample(Xe, y_edge)
                pca2     = PCA(n_components=min(Xe2.shape[1]-1,5)).fit(Xe2)
                Xe3      = pca2.transform(Xe2)
                svc2     = SVC(kernel='linear', class_weight={0:1, 1:5}, probability=True).fit(Xe3, ye2)

                # — now evaluate this manual‐mode specialist on test edge‐cases —
                skew_test      = df_clean['skewF'].iloc[test_idx]
                mask_edge_test = (skew_test > skew_edge_low) & (skew_test < skew_edge_high)
                if mask_edge_test.any():
                    X_edge_test = X_test.loc[mask_edge_test]
                    y_edge_test = y_test.loc[mask_edge_test]
                    Xe_test     = pca2.transform(sc2.transform(X_edge_test))
                    y_edge_pred = svc2.predict(Xe_test)
                    print("Edge‐case specialist test accuracy:", accuracy_score(y_edge_test, y_edge_pred))
                    print(classification_report(y_edge_test, y_edge_pred))
    
                    # and now inspect different thresholds on the class‑1 probability:
                    probs_true = svc2.predict_proba(Xe_test)[:,1]
                    for t in [0.2, 0.3, 0.4, 0.5]:
                        pred_t = (probs_true > t)
                        print(f"\n— threshold = {t} —")
                        print(classification_report(y_edge_test, pred_t))
    
                # stash your manual specialist so predict_cell can find it:
                self.edge_scaler   = sc2
                self.edge_smote    = sm2
                self.edge_pca      = pca2
                self.edge_svc      = svc2
    ...
    def gather_classifier_data(self, classifier_sessions):
        ...
        for fpath in classifier_sessions:
            ...
            fileName = os.path.split(os.path.split(os.path.split(fpath)[0])[0])[-1]
            ...
            fileName_array = np.array([fileName] * F.shape[0])
            cellIndex_array =  np.arange(F.shape[0], dtype=int)
    ...
    def cleanup_classifier_data(self, df_all, drop_nan: bool = True):
    ...
    def predict_cell(self, df_predict):
        ...
        then override any skew‑edge cases with the secondary classifier.
        """
        import numpy as np
        ...
        if getattr(self, 'pipeline', None) is not None:
            ...
            # manual flow: scale → PCA → SVC
            Xs = self.scaler.transform(X)
            if self.pca is not None:
                Xp = self.pca.transform(Xs)
            else:
                Xp = Xs
            preds  = self.svc.predict(Xp)
            probs  = self.svc.predict_proba(Xp)
            scores = self.svc.decision_function(Xp)

        # 4) Edge‑case override
        # assume you stored these on build_classifier:
        low, high = self.skew_edge_low, self.skew_edge_high
        mask_edge = (skew_vals > low) & (skew_vals < high)

        if mask_edge.any() & self.skew_classifier==True:
            X_edge = X.loc[mask_edge]

            # grid_search‐mode specialist
            if getattr(self, 'edge_pipeline', None) is not None:
                # grab the probability of the "True cell" class
                p1 = self.edge_pipeline.predict_proba(X_edge)[:,1]
                # override preds at p1 > 0.5
                preds[mask_edge] = (p1 > 0.5)
                # write the new probability vector back in
                probs[mask_edge,1] = p1
                probs[mask_edge,0] = 1 - p1
                # (optionally) update scores if you want to keep the decision_function
                if hasattr(self.edge_pipeline, 'decision_function'):
                    scores[mask_edge] = self.edge_pipeline.decision_function(X_edge)

            # manual‐mode specialist
            elif hasattr(self, 'edge_svc'):
                Xe    = self.edge_scaler.transform(X_edge)
                Xe    = self.edge_pca.transform(Xe)
                p1    = self.edge_svc.predict_proba(Xe)[:,1]
                preds[mask_edge]  = (p1 > 0.5)
                probs[mask_edge,1] = p1
                probs[mask_edge,0] = 1 - p1
                scores[mask_edge] = self.edge_svc.decision_function(Xe)

        # 5) Re‑insert dropped rows as False/0
        if len(idx_rem) > 0:
            n_total = len(preds) + len(idx_rem)
            final_preds  = np.zeros(n_total,             dtype=bool)
            final_probs  = np.zeros((n_total, probs.shape[1]))
            ...
            for i in range(n_total):
                if i in idx_rem:
                    continue
                final_preds[i]  = preds[pred_i]
                final_probs[i]  = probs[pred_i]
                final_scores[i] = scores[pred_i]
                pred_i += 1

            preds, probs, scores = final_preds, final_probs, final_scores
    ...
    def rewrite_data(self, predict_sessions, predictions, probabilities):
        ...
        for i in predict_sessions:
            ...
            print("Tossing cells if their skew < 0.9")
            iscell_out[ (skF < 0.9) | (skFc < 0.9) ] = False
...
def calcium_events(Fc,
    ...
    peaks : ndarray of int
        Sample indices of detected calcium peaks.
    widths_s : ndarray of float
        Peak full widths at half‐height, in seconds.
    mean_rise_s : float
        Mean rise time (half‐height to peak), in seconds.
    mean_fall_s : float
        Mean fall time (peak to half‐height), in seconds.
    iei_s : ndarray of float
        Inter‐event intervals (seconds).
    amps : ndarray of float
        Peak heights above baseline.
    props : dict
        Full `find_peaks` properties dict (e.g. `'peak_heights'`).
    asymmetry : float
        Median absolute difference between rise and fall times.
    width_var : float
        Variance of peak widths (s²), or NaN if zero.
    event_rate : float
        Number of peaks per second (Hz).
    pnr : float
        Mean peak‐to‐noise ratio (height divided by 99th‐percentile noise).
    snr_power : float
    ...
    # now replace events with nan
    noise = Fc.copy()
    noise[ttimes] = np.nan # nan out the events
    
    # interpolate candidate signal events to estimate noise
    noise = np.interp(np.arange(len(noise)), np.arange(len(noise))[~np.isnan(noise)], noise[~np.isnan(noise)])
    sn = np.std(noise) # std of the noise (baseline) distribution
    ...
    widths, _, left_ips, right_ips = peak_widths(Fc, peaks, rel_height=0.75)
    widths_s = widths / fs
    rise_s   = (peaks - left_ips)  / fs
    fall_s   = (right_ips - peaks) / fs

    # 3b) drop any peaks missing a valid rise *or* fall interval
    valid = (~np.isnan(left_ips)) & (~np.isnan(right_ips)) & (rise_s>0) & (fall_s>0)
    peaks    = peaks[valid]
    widths_s = widths_s[valid]
    rise_s   = rise_s[valid]
    fall_s   = fall_s[valid]  

    # width variance
    width_var = np.var(widths_s)  
    if width_var==0:
        width_var=np.nan
    ...
    # number of events
    event_rate = peaks.shape[0] / (Fc.shape[0]/fs)

    # pnr: the average of peak/99th percentile of noise distribution
    pnr = np.mean(amps / noise_top)

    # get power spectrum of noise
    nperseg = int(4 * fs)
    freqs, Pxx_signal = welch(Fc,    fs=fs, nperseg=nperseg, noverlap=nperseg//2)
    freqs, Pxx_noise  = welch(noise, fs=fs, nperseg=nperseg, noverlap=nperseg//2)
    power_ratio=Pxx_signal/Pxx_noise

    # after computing freqs, power_ratio...
    target = 0.25
    idx = np.argmin(np.abs(freqs - target))
    snr_power = power_ratio[idx]
    #power_ratio_avg = power_ratio[freqs==.25][0]

    # Diagnostic plotting
    if plot_progress:
        #%matplotlib qt
        plt.figure()
        plt.plot(Fc_old,   color='k', label='Detrended/Neuropil Corrected F', linewidth=.5)
        plt.scatter(peaks, Fc_old[peaks],color='r', label='Peaks')
        y_left  = np.interp(left_ips,  np.arange(len(Fc_old)), Fc_old)
        y_right = np.interp(right_ips, np.arange(len(Fc_old)), Fc_old)
        ...
        plt.figure()
        plt.semilogy(freqs, Pxx_signal / Pxx_noise)   # log‐scale y makes broad peaks easier to see
        plt.xlabel('Frequency (Hz)')
        plt.ylabel('PSD signal / noise')
        plt.title("Power Spectral Density Ratio")
        plt.xlim(0, fs/2)
        plt.tight_layout()
        plt.show()

    return peaks, widths_s, np.mean(rise_s), np.mean(fall_s), iei, amps, props, asymmetry, width_var, event_rate, pnr, snr_power
