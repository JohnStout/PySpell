...
def validate_modeling_steps(folder_path, idx_fixed=0, idx_moving=1):
    ...
    # Limit sessions for speed (mirrors MATLAB validator max_sessions_modeling)
    n_use = min(4, len(cellreg_files))
    cellreg_files_use = cellreg_files[:n_use]

    # Load footprints
    spatial_footprints = []
    for f in cellreg_files_use:
        fp = get_spatial_footprints(f)
        spatial_footprints.append(fp)
    
        # Normalize + adjust FOV sizes
    ...
    # Compute centroids + projections (needed for alignment/diagnostics)
    print("Computing centroids...")
    centroid_locations = compute_centroids(adjusted_fps, config.microns_per_pixel)
    adjusted_fp_projs = compute_footprint_projections(adjusted_fps)

    # ------------------------------------------------------------------
    # CRITICAL: Align/Correct footprints using MEAN-IMAGE alignment
    # This mirrors the production pipeline in batchRunCellReg:
    # mean-image alignment -> apply transform to spatial footprints/centroids
    # ------------------------------------------------------------------
    print("\nApplying mean-image alignment transforms to footprints (correction step)...")

    # Mean images must correspond to the SAME sessions/order as cellreg_files_use
    mean_images_alignable = [get_mean_image(f.parent, apply_drift_correction=True) for f in cellreg_files_use]

    # Choose reference session for alignment (default: idx_fixed, else 0)
    ref_idx = idx_fixed if 0 <= idx_fixed < len(mean_images_alignable) else 0
    print(f"  Reference session for correction: {ref_idx} (0-based within modeled set)")

    aligned_fps = [None] * len(adjusted_fps)
    aligned_centroids = [None] * len(centroid_locations)
    align_methods = [None] * len(adjusted_fps)
    align_peaks = np.full(len(adjusted_fps), np.nan)

    # Reference stays unchanged
    aligned_fps[ref_idx] = adjusted_fps[ref_idx]
    aligned_centroids[ref_idx] = centroid_locations[ref_idx]
    align_methods[ref_idx] = "reference"
    align_peaks[ref_idx] = 1.0

    for i in range(len(adjusted_fps)):
        if i == ref_idx:
            continue

        # Align mean images (moving=i) to reference mean image
        _, method, peak, tform, _, _ = cellreg.aligner.align(
            mean_images_alignable[ref_idx],
            mean_images_alignable[i],
            filter_mode='highpass',
            outlier_mode='off'
        )
        align_methods[i] = method
        align_peaks[i] = peak
        print(f"  Session {i}: method={method}, peak={peak:.4f}")

        if (tform is None) or (peak < config.alignable_threshold):
            # Not alignable: leave as-is (but still keep array defined)
            aligned_fps[i] = adjusted_fps[i]
            aligned_centroids[i] = centroid_locations[i]
            continue

        # Warp each footprint to reference space
        fps = adjusted_fps[i]
        warped = np.zeros_like(fps, dtype=np.float32)
        for k in range(fps.shape[0]):
            warped[k] = sktransform.warp(
                fps[k],
                tform.inverse,
                output_shape=fps[k].shape,
                order=1,
                preserve_range=True,
                mode='constant',
                cval=0.0
            ).astype(np.float32)
        aligned_fps[i] = warped

        # Transform centroid locations to reference space
        cents = centroid_locations[i]
        if cents is None or len(cents) == 0:
            aligned_centroids[i] = cents
        else:
            coords = np.column_stack([cents[:, 0], cents[:, 1], np.ones(len(cents))])
            xy = (tform.params @ coords.T).T
            aligned_centroids[i] = xy[:, :2]

    # Overwrite downstream variables (so the rest of the validator is unchanged)
    adjusted_fps = aligned_fps
    centroid_locations = aligned_centroids
    adjusted_fp_projs = compute_footprint_projections(adjusted_fps)
