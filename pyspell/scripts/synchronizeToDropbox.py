...
files_to_copy = ['stat.npy', 'spks.npy', 'F.npy', 
                 ...
                 'CellReg.mat', 'F_clean.mat', 'beh.mat', 'Experiment.xml']
#files_to_copy = ['beh.mat', 'Experiment.xml']

# it's a good tactic against your enemies
force_push = True # force push from storage to dropbox
force_pull = False # force pull from dropbox to storage
...
def synchronize_folders(origin: str, destination: str, files_to_copy: list):
    ...
    for filename in files_to_copy:
        if filename == 'CellReg.mat':
            # search for the cellreg file
            filename = [k for k in os.listdir(origin) if 'CellReg' in k]
            if len(filename) > 0:
                filename = filename[0]
            else:
                print("No CellReg file found.")
                continue
        else:
...
for i in syncpaths:
    ...
    behfolders = [i for i in subdirs if 'beh' in i]

    # check if the dropbox folder exists
    if os.path.exists(i['Dropbox']) == False:  
        print("Creating dropbox folder:",i['Dropbox'])
        os.makedirs(name = i['Dropbox'], exist_ok = True)
    ...
    for fpath in datafolders:
        ...
        behfolder = [k for k in os.listdir(rname) if 'beh' in k]
        ...
        # search for beh.mat
        beh_in_list = [j for j in files_to_copy if 'beh' in j]
        ...
        for bpi in behfolder:
            ...
            behfound = [k for k in os.listdir(dropbox_folder) if 'beh' in k and '.mat' in k]

            # check for the true beh.mat file name
            behfile_in_origin = [k for k in bfiles if 'beh' in k and '.mat' in k]
            if len(behfile_in_origin) > 0:
                behfile_in_origin = behfile_in_origin[0]
            else:
                behfile_in_origin = None
                beh_in_list = []

            # if there is existing behavioral data and you do NOT wish to force push to dropbox from origin, synchronize
            if len(behfound) > 0 and force_push == False and force_pull == False and len(beh_in_list) > 0 and behfile_in_origin != None:

                # search for update
                mod_date_dropbox, mod_time_dropbox = get_date_time(file_path = os.path.join(dropbox_folder, behfile_in_origin))

                # get origin date
                mod_date_origin, mod_time_origin = get_date_time(file_path = os.path.join(bpath, behfile_in_origin))
                ...
                if mod_date_dropbox != mod_date_origin or mod_time_dropbox != mod_time_origin:
                    ...
                    if mod_time_dropboxdt > mod_time_origindt and offset > timedelta(minutes=2):
                        ...
                        synchronize_folders(origin = dropbox_folder, destination = bpath, files_to_copy = [behfile_in_origin])
                        ...
                        synchronize_folders(origin = bpath, destination = dropbox_folder, files_to_copy = [behfile_in_origin])
            
            else:
                if force_push == True and len(beh_in_list) > 0:
                    print("Good. Good. Now finish it. Pushing behavior from origin to dropbox.")

                    # copy the file based on recent updates
                    synchronize_folders(origin = bpath, destination = dropbox_folder, files_to_copy = [behfile_in_origin])

                elif force_pull == True and len(beh_in_list) > 0:
                    synchronize_folders(origin = dropbox_folder, destination = bpath, files_to_copy = [behfile_in_origin])
        ...
        # update the cellreg name
        target_list = os.listdir(fpath) # target list are the files within the origin folder (your drive, not dropbox)
        cell_reg_search = [fi for fi in target_list if 'CellReg'.lower() in fi.lower()]
        if len(cell_reg_search) > 0:
            cell_reg_name = cell_reg_search[0]
        for si in range(len(files_to_copy)):
            if 'CellReg'.lower() in files_to_copy[si].lower():
                files_to_copy[si] = cell_reg_name
        ...
        if file_check == False or force_push == True or force_pull == True:
            if force_push == True:
                ...
                if 'Experiment.xml' in files_to_copy:

                    # copy the behavior file
                    synchronize_folders(origin = fpath.split('\\suite2p')[0], destination = dropbox_s2p_folder.split("\\suite2p")[0], files_to_copy = ['Experiment.xml'])
    
            elif force_pull == True:
                ...
                if 'Experiment.xml' in files_to_copy:
                    ...
                    # copy the behavior file
                    synchronize_folders(origin = dropbox_s2p_folder.split("\\suite2p")[0], destination = fpath.split('\\suite2p')[0], files_to_copy = ['Experiment.xml'])
        ...
        if force_push == False and force_pull == False:
